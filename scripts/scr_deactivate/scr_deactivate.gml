...
function scr_deactivate(_instance = self){
	...
	show_debug_message("scr_deactivate: Deactivating " + object_get_name(_instance.object_index) + "[" + string(real(_instance)) + "]");
	...
	if (_instance.state.previous_active != noone) && instance_exists(_instance.state.previous_active) && _instance.state.previous_active.state.standby {
		show_debug_message("scr_deactivate: Deactivated object had a memorized object on standby: " 
		+ object_get_name(_instance.state.previous_active.object_index)
		+ "[" + string(real(_instance)) + "]");
